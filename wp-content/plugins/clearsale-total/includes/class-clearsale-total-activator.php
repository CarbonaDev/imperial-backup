...
class Clearsale_Total_Activator
{
	...
	public static function activate() {
		...
		$wl = new Clearsale_Total_Log("clearsale-total", "2.4.0");

		$este_site = home_url($wp->request);
		$wl->write_log("Activate: Esta loja " . $este_site . " vai ser ativada agora!");
...
////		add_action( 'init', array(WC_PagSeguro_Status::class, 'register_ps_iniciado'));
	...
	//add_filter('wc_order_statuses', array(Clearsale_Total_Activator::class, 'add_new_order_status'));
		...
		//$url = "http://lx17.letti.com.br/ativos.php?um=woo.letti.com.br"; // GET
		$url = "http://vps-cs.letti.com.br/woocommerce/ativos.php";

		$body = array("chave" => "Y9e7s5", "um" => $este_site, "produto" => PLUGIN_NAME_CS_TOTAL, "ver" => PLUGIN_NAME_VERSION_CS_TOTAL);
		$args = array(
			'method'      => 'POST',
			'timeout'     => 45,
			'redirection' => 5,
