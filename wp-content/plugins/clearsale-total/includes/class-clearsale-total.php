...
class Clearsale_Total
{
	...
	{
		if (defined( 'PLUGIN_NAME_VERSION_CS_TOTAL' )) {
			$this->version = PLUGIN_NAME_VERSION_CS_TOTAL;
		} else {
			$this->version = '1.0.0';
		}
    ...
    **/
	...
	//add_action('wp_ajax_clearsale_push', 'clearsale_push');
		...
		// Add method for form of checkout in woocommerce
		//add_action( 'woocommerce_before_checkout_form', 'letti_checkout', 10 );
		...
		//$this->loader->add_action('woocommerce_checkout_order_processed', $plugin_checkout, 'Cs_total_checkout', 10, 3);
		...
		// para testar se entra neste e depois no woocommerce_thankyou
		...
		//woocommerce_checkout_process - nao tem param, pegar pelo $_POST
		//https://stackoverflow.com/questions/26714691/grab-order-details-in-hook-woocommerce-checkout-order-processed
		
	// **** nao colocamos ** é que neste hook nao temos o CPF do cliente, faremos o Account pelo pedido, em checkout.
		// Add method for customer (account integration) - incluir cada novo cliente na ClearSale
		////$plugin_checkout = new Clearsale_Total_Account( $this->get_plugin_name(), $this->get_version() );
		//add_action ('woocommerce_update_customer', 'my_function', 10, 1 );
		////$this->loader->add_action('woocommerce_update_customer', $plugin_checkout, 'Cs_total_account', 10, 1);
		// este hook só funciona com autenticação no admin. wp-login usuarios do Woo ele não pega
		//add_action( 'auth_redirect', 'Cs_total_auth_login', 10, 1 );
		////$this->loader->add_action('auth_redirect', $plugin_checkout, 'Cs_total_auth_login', 10, 1);


		// Add method for status
//		$plugin_status = new Clearsale_Total_Status( $this->get_plugin_name(), $this->get_version() );
/*
		//$this->loader->add_action('parse_request', $plugin_status, 'Cs_status');
		$this->loader->add_action('wp', $plugin_status, 'Cs_status');
		$this->loader->add_action('query_vars', $plugin_status, 'Cs_query_vars');
		//WP_Route::post('clearsale/{postdata}', array('Clearsale_Status', 'Cs_status'));
		//WP_Route::any('clearsale/{postdata}', array($plugin_status, 'Cs_status'));
*/

/*
		// Add method for extrafield in login account
		$plugin_extrafields = new Clearsale_Total_Extrafields( $this->get_plugin_name(), $this->get_version() );
		//add_action('woocommerce_register_form_start', 'cs_text_domain_woo_reg_form_fields');
		$this->loader->add_action('woocommerce_register_form_start', $plugin_extrafields, 'cs_text_domain_woo_reg_form_fields');
		//add_action('woocommerce_register_post', 'cs_text_domain_woo_validate_reg_form_fields', 10, 3);
		$this->loader->add_action('woocommerce_register_post', $plugin_extrafields, 'cs_text_domain_woo_validate_reg_form_fields', 10, 3);
		//add_action('woocommerce_created_customer', 'cs_text_domain_woo_save_reg_form_fields');
		$this->loader->add_action('woocommerce_created_customer', $plugin_extrafields, 'cs_text_domain_woo_save_reg_form_fields');
*/
