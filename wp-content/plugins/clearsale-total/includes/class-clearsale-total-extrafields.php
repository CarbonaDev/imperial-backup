...
class Clearsale_Total_Extrafields
{
    ...
    */
    ...
    function cs_customise_checkout_field($checkout)
    {
	    ...
	    echo '<div id="customise_checkout_field"><h3>' . __('Document Number', $this->plugin_name) . '</h3>';
	    woocommerce_form_field('cs_field_doc', array(
		    'type' => 'text',
		    'class' => array(
    			'cs-field-class form-row-wide'
	    	) ,
		    'label' => __('Cpf/Cnpj (Valid only in Brazil)', $this->plugin_name) ,
		    'placeholder' => __('Only numbers', $this->plugin_name) ,
    ...
    function cs_customise_checkout_field_process()
    {
        ...
        if ($b_country == "BR") { // && $s_country == "BR") {
            ...
            $doc = $_POST['cs_field_doc'];
            if (!$doc) {
                wc_add_notice(__('Please enter value in Document Number.', $this->plugin_name), 'error');
            }
            if (strlen($doc) <= 11) {
                if (!$this->validaCPF($doc)) {
                    wc_add_notice(__('Invalid CPF Number.', $this->plugin_name), 'error');
                }
            } elseif (!$this->validaCNPJ($doc)) {
                wc_add_notice(__('Invalid CNPJ Number.', $this->plugin_name), 'error');
            }
        }
    }
    ...
    */
    ...
    function cs_customise_checkout_field_update_order_meta($order_id)
    {
        ...
        if (isset($_POST['cs_field_doc'])) {
            $tmp = sanitize_text_field($_POST['cs_field_doc']);
        }
        $b_cpf = $b_cnpj = "";
        if (isset($_POST['billing_cpf'])) $b_cpf = $_POST['billing_cpf'];
        if (isset($_POST['billing_cnpj'])) $b_cnpj = $_POST['billing_cnpj'];
        ...
        if (!empty($_POST['cs_field_doc'])) { // grava no postmeta com nome cs_doc
            update_post_meta($order_id, 'cs_doc', sanitize_text_field($_POST['cs_field_doc']));
        } else {
            ...
            $tmp_cpf = $_POST['billing_cpf'];
            $tmp_cnpj = $_POST['billing_cnpj'];
            if (strlen($tmp_cpf) < 5) {
                $n_doc = $tmp_cnpj;
            } else {
                $n_doc = $tmp_cpf;
