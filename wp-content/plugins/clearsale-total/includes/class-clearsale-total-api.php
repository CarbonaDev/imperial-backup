...
class Clearsale_Total_Api
{
	...
	*
	* @return	string	token se exito ou false se erro no curl
	*/
...
//		$this->wl->write_log("Authenticate: agora=" . $dt_agora . "  expirat=" . $this->expirationDate);
		...
		$url = $tmp . $servico; // endpoint final para authentication
		...
		if (curl_errno($ch)) {
			...
			$this->wl->write_log("Authenticate: erro no curl=" . curl_error($ch));
			return false;
		}
	...
	* Este método vai ficar fora do contexto, vai ser chamado pelo Cs_status.
	* 
	...
	*/
	public function Consulta_status($pedido)
	{
		...
		$url = $tmp . $servico; // endpoint final para inclusão de pedidos
		...
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_HEADER, 1);	//separa header e body

		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
		curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);

		$ret = curl_exec($ch);
		$erro_c = curl_errno($ch);
		if ($erro_c) {
			//echo 'Error:' . curl_error($ch);
			$this->wl->write_log("Consulta_status: erro no curl=" . curl_error($ch));
			return false;
		}
		...
		list($lixo, $requestID) = explode("Request-ID:", $header);
		$requestID = substr($requestID, 0, 20);

		if ($http_code != 200) {
			$this->wl->write_log("Consulta_status: erro #" . $http_code . " msg=" . print_r($body,true));
			return false;
		}
		...
		$this->wl->write_log("Consulta_status: #pedido=" . $code . " status=" . $status . " score=" . $score);
	...
	*/
	public function Atualiza_status($pedido, $cs_status)
	{
		...
		if ($erro_c) {
			//echo 'Error:' . curl_error($ch);
			$this->wl->write_log("Atualiza_status: erro no curl=" . curl_error($ch));
			return false;
		}
		$http_code = curl_getinfo($ch,CURLINFO_HTTP_CODE);
		$header_size = curl_getinfo($ch, CURLINFO_HEADER_SIZE);
...
$this->wl->write_log("Atualiza_status: ret=" . print_r($json_str,true));
	...
	*/
	public function Accounts($customer)
	{
		...
		$url = $tmp . $servico; // endpoint final para accounts
		$token = $this->Authenticate();	// retorna token
		$ch = curl_init();
		...
		if ($erro_c) {
			//echo 'Error:' . curl_error($ch);
			$this->wl->write_log("Accounts: erro no curl=" . curl_error($ch));
			return false;
		}
		$http_code = curl_getinfo($ch,CURLINFO_HTTP_CODE);
		$header_size = curl_getinfo($ch, CURLINFO_HEADER_SIZE);
...
*/
	...
	*/
	...
	/**
	* Metodo para incluir ChargeBack na ClearSale
	* 
	* @param	string	$pedido - # pedido
	* @param	string	$msg
	* @return	mixed false|array - False se der erro no curl, array com retorno array('requestID'=>$requestID, 'code'=>$code, 'status'=>$status, 'score'=>$score)
	*/
	public function Chargeback($pedido, $msg)
	{

		$servico = "/v1/chargeback";
		if ($this->modo) $tmp = $this->_endp_h;	//homologação
		else $tmp = $this->_endp_p;	//produção

		$body_json = '{ "message" : "' . $msg . '", "orders" : ["' . $pedido . '"] }';

		$url = $tmp . $servico; // endpoint final para chargeback
		$token = $this->Authenticate();	// retorna token
		$ch = curl_init();
		...
		if ($erro_c) {
			//echo 'Error:' . curl_error($ch);
			$this->wl->write_log("Chargeback: erro no curl=" . curl_error($ch));
			return false;
		}
		$http_code = curl_getinfo($ch,CURLINFO_HTTP_CODE);
		$header_size = curl_getinfo($ch, CURLINFO_HEADER_SIZE);
		...
		if ($http_code != 200) {
			$this->wl->write_log("Chargeback: erro #" . $http_code . " msg=" . print_r($body,true));
			return false;
		}
/* retorno do curl
[
...
*/
		curl_close ($ch);
		$json_str = json_decode($body, true);
$this->wl->write_log("Chargeback: retorno=" . print_r($json_str,true));
		$ret = $json_str[0];
		$code = $ret['code'];		// # pedido
		$status = $ret['status'];	// Chargeback done

		$this->wl->write_log("Chargeback: #pedido=" . $code . " status=" . $status);
		return array('requestID'=>$requestID, 'code'=>$code, 'status'=>$status);
	...
	*/
	public function Inclui_pedido($pedido_json)
	{
		...
		if ($erro_c) {
			//echo 'Error:' . curl_error($ch);
			$this->wl->write_log("Inclui_pedido: erro no curl=" . curl_error($ch));
			return false;
		}
		$http_code = curl_getinfo($ch,CURLINFO_HTTP_CODE);
		$header_size = curl_getinfo($ch, CURLINFO_HEADER_SIZE);
...
/* como nao tem dados o Joelson pediu para tirar - email 3/12/18
			...
			*/
			...
			if ("pagseguro" === $order_data['payment_method'] && !$cartao['modo']) {//pagseguro mas nao oficial 26/7/20
				...
				//$tipo = get_post_meta($order_data['id'], "Tipo de pagamento", true); //Cartão de Crédito
...
*/
	...
	*/
	public function Tipo_pagamento($tipo)
	{
		...
		if (strstr($texto, "loja5_woo_novo_erede")) return(1); // crédito, pelo log da vistavie.com.br
		...
		if (strstr($texto, "ipag-gateway_debito")) return(3); // boleto
		if (strstr($texto, "ipag-gateway_itaushopline")) return(2); // boleto poder debito tb
	...
	*/
	public function Busca_Bairro_ws($cep)
	{
		...
		try
		{
			...
			$clientSoap = new SoapClient("https://apps.correios.com.br/SigepMasterJPA/AtendeClienteService/AtendeCliente?wsdl",$soapparam);
			$result = $clientSoap->consultaCep($soapArgs);
			$dados = $result->return;

            if (is_soap_fault($result)) {
				$return = "    ";
				$this->wl->write_log("Clearsale_Total_Api:Busca_Bairro: erro soap_fault");
            } else {
				$return = $dados->bairro;
            }
		...
		{
			$return = "    ";
			$this->wl->write_log("Clearsale_Total_Api:Busca_Bairro: erro SOAP=" . $e->faultcode . " str=" . $e->faultstring);
		}
		catch (Exception $e)
		{
			$return = "    ";
			$this->wl->write_log("Clearsale_Total_Api:Busca_Bairro: erro exception=" . $e->getMessage());
        }

		if (strlen($return) <= 3) $return = "    ";
		$this->wl->write_log("Clearsale_Total_Api:Busca_Bairro: saida=" . $return);
		return $return;
