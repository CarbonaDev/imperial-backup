...
class Clearsale_Total_Requirements
{
    ...
    */
    public function __construct($plugin_name, $version, $plugin_file, $requirements)
    {
        ...
        if (empty( $requirements ) || ! is_array( $requirements )) {
            trigger_error( __('WP Requirements: the requirements are invalid.', $plugin_name), E_USER_ERROR );
        }
    ...
    */
    public function getNotice( $message = '' )
    {
        ...
        $notice .= "\t" . '<p>' . "\n";
        //$notice .= '<strong>' . sprintf( '%s', $name) . __(' could not be activated.', $name) . '</strong><br>';
        $notice .= '<h3>' . sprintf( '%s', $name) . __(' could not be activated.', $name) . '</h3><br>';
        ...
        foreach ($failures as $requirement => $found) {
            ...
            if ('Extensions' == $requirement) {
                if ( is_array( $found ) ) {
                    $notice .= __('Required PHP Extension(s) not found: ', $name) .
...
//      $notice .= '<em>' . sprintf( 'Please update to meet %s requirements.', $name ) . '</em>' . "\n";
        $notice .= '<em>' . __('Please update to meet requirements.', $name) . '</em>' . "\n";
