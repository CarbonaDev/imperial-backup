...
class WC_Simulador_Parcelas {
  ...
  public function enqueue_css() {
    if ( apply_filters( 'wcsp_load_css', true ) ) {
      $style = get_option( 'wc_simulador_parcelas_style', 'default' );

      if ( ! in_array( $style, array_keys( wcsp_get_available_styles() ) ) ) {
        $style = 'default';
      }
      ...
      $files = array(
        'default'     => 'default.css',
        'default_pix' => 'default-pix.css',
      );

      wp_enqueue_style( 'wc-simulador-parcelas-css', plugins_url( '/assets/css/' . $files[ $style ], __FILE__ ), 999 );
    }
  }
