...
class WCSP_Apply_Rules extends WC_Simulador_Parcelas_Init {
  ...
  */
  public function single_product_price( $product, $original_price = false ) {
    ...
    $price    = $this->set_values( $exibicao, wc_get_price_to_display( $product, $args ), $product, false );
  ...
  public function woocommerce_get_price_html( $price, $product ) {
    ...
    if ( 0 < self::get_main_price_discount( $product ) && in_array( $ticket_visibility, array( 'both', $hook ) )
      ...
      if ( $product->is_type( 'variable', 'variation' ) && ! $this->variable_has_same_price( $product ) ) {
        ...
        // em alguns cenários, quando $get_variations é false, o menor preço parece não ser retornado em $product->get_price()
        $args['price'] = $product->get_variation_price( 'min' );
      }

      $custom_price = WC_Simulador_Parcelas_Calculate_Values::calculate_discounted_price( wc_get_price_to_display( $product, $args ), $main_price_discount, $product );
